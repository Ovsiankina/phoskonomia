...
const PHOSK_PAGES = [
  { key: "DASHBOARD",     abbr: "DASH", glyph: "◳", href: "Phoskonomia%20Dashboard.html",     desc: "Spend trace, savings & alerts" },
  { key: "TRANSACTIONS",  abbr: "TXN",  glyph: "⊟", href: "Phoskonomia%20Transactions.html",  desc: "Every receipt, itemized" },
  { key: "BUDGETS",       abbr: "BUDG", glyph: "▦", href: "Phoskonomia%20Budgets.html",       desc: "Envelopes & monthly caps" },
  { key: "SUBSCRIPTIONS", abbr: "SUBS", glyph: "⊠", href: "Phoskonomia%20Subscriptions.html", desc: "Standing recurring charges" },
  { key: "DEBTS",         abbr: "DEBT", glyph: "∿", href: "Phoskonomia%20Debts.html",         desc: "Balances, payoff & IOUs" },
  { key: "ANALYTICS",     abbr: "ANLY", glyph: "⌁", href: "Phoskonomia%20Analytics.html",     desc: "Trends & item-signals" },
  { key: "CONFIG",        abbr: "CFG",  glyph: "⊙", href: "Phoskonomia%20Config.html",        desc: "Preferences for every surface" },
];
...
function TopBar({ active = "DASHBOARD" }) {
  ...
  const dateText = String(dateFmt || "")
    .replace(/\{label\}/g, D.cycle.label)
    .replace(/\{day\}/g, D.cycle.day)
    .replace(/\{days\}/g, D.cycle.days)
    .replace(/\{asOf\}/g, D.cycle.asOf || "")
    || " ";
  ...
  return (
    <header className="phosk-top">
      <a className="phosk-brand" href="Phoskonomia%20Dashboard.html" aria-label="Phoskonomia — home">
      ...
      <div className={"phosk-navwrap" + (compact ? " compact" : "")} ref={navwrapRef}>
        ...
        <nav className="phosk-nav" onMouseLeave={onNavLeave}>
          {PHOSK_PAGES.map((p) =>
          <a key={p.key} className={"t" + (p.key === active ? " on" : "")} href={p.href}
            onMouseEnter={(e) => onNavEnter(e, p)} onFocus={(e) => onNavEnter(e, p)}>{navMode === "abbr" ? p.abbr : p.key}</a>
          )}
        </nav>}
      ...
      <window.HudCell glyph="P" size={38} />

      {/* inline hover-card (expanded nav only) — full name + what the page is for */}
      {!compact && hoverPage && ReactDOM.createPortal(
              ...
              {PHOSK_PAGES.map((p) => {
                ...
                const inner = (
                  <React.Fragment>
                    ...
                    {on ? <span className="mega-cur">● ACTIVE</span> : p.href ? null : <span className="soon">SOON</span>}
                  </React.Fragment>
                );
                return p.href
                  ? <a key={p.key} className={"mega-card" + (on ? " on" : "")} href={p.href} role="menuitem" onClick={() => setMenu(false)}>{inner}</a>
  ...
  return (
    <>
      {cats.map((c) => {
        ...
        const tone = c.fixed ? "ok" : window.pctTone(p);
        return (
          <div key={c.name} className="catrow">
            <span className={"cn" + (c.fixed ? " fixed" : "")}>{c.name}</span>
            <div className="barwrap">
              <window.CatBar spent={c.spent} budget={c.budget} tone={tone} />
              <div className="barmeta">
                <span>{showItems ? c.items + " items" : c.fixed ? "FIXED" : "VARIABLE"}</span>
                <span>{c.budget === 0 ? "NO BUDGET" : "CAP CHF " + D.chf(c.budget, 0)}</span>
              </div>
            </div>
            <span className="amt">CHF <b>{D.chf(c.spent)}</b></span>
  ...
  return (
    <>
      ...
      <div key={i} className="txn">
          ...
          <span className="tag"><window.Dot tone={t.fixed ? "blue" : "ok"} size={5} />{t.cat}</span>
          <span className="am"><span className="c">CHF</span>{D.chf(t.amount)}</span>
        </div>
      )}
    </>);
...
function RecRow({ r }) {
  ...
  return (
    <div className="rec">
      <window.Dot tone={tone} size={8} />
      <div className="body">
        <span className="nm">{r.name}</span>
        <span className="cy">{r.cycle}</span>
      </div>
      <div className="right">
        <span className="am">CHF {D.chf(r.amount)}</span>
...
function AlertItem({ a }) {
  ...
  // Map each alert action label to its backend endpoint (paths use the alert id).
  const runAction = (act) => {
    switch (act) {
      case "APPLY":
        return window.phoskApi.post("/alerts/" + a.id + "/apply", { new_budget: a.new_budget, move_to_savings: a.move_to_savings });
      case "RAISE CAP":
      case "LOWER CAP":
        return window.phoskApi.post("/alerts/" + a.id + "/apply", { new_budget: a.new_budget });
      case "DISMISS":
        return window.phoskApi.post("/alerts/" + a.id + "/dismiss");
      case "SNOOZE":
        return window.phoskApi.post("/alerts/" + a.id + "/snooze", { duration: a.snoozeDuration });
      case "MARK PAID":
        return window.phoskApi.post("/recurring/" + (a.relatedRecurring || a.tag) + "/mark-paid");
      case "VIEW":
        return window.phoskApi.get("/alerts/" + a.id + "/target");
      default:
        return window.phoskApi.post("/alerts/" + a.id + "/dismiss");
    }
  };
  return (
    <div className={"alert-i " + a.tone}>
      ...
      <div className="main">
        ...
        <div className="acts">
          {a.actions.map((act, i) => <button key={i} className={"btn" + (i === 0 ? " p" : "")} onClick={() => runAction(act)}>{act}</button>)}
        </div>
      </div>
    </div>);
...
Object.assign(window, { TopBar, Kpi, CatRows, TxnTape, RecRow, AlertItem });