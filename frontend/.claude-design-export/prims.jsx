/* Phoskonomia — shared primitives. Exported to window for cross-file use. */
...
function PhoskChart({ width, height, showBars = true, showPace = true, showArea = true, showLast = false, padL = 8, padR = 8, padT = 14, padB = 18 }) {
  const D = window.PHOSK;
  const days = D.cycle.days,today = D.cycle.day;
  const W = width,H = height;
  const x = (d) => padL + d / (days - 1) * (W - padL - padR);
  const maxY = D.totals.budget * 1.04;
  ...
  const lastX = x(cum.length - 1),lastY = y(cum[cum.length - 1]);

  // budget pace
  const pacePts = D.pace.map((v, i) => `${x(i).toFixed(1)},${y(v).toFixed(1)}`).join(" ");

  // synthetic prior-cycle cumulative for comparison (scope mode)
  const lastCum = (() => {let a = 0;return D.daily.map((d) => a += d * 1.07);})();
  const lastPts = lastCum.map((v, i) => `${x(i).toFixed(1)},${y(v).toFixed(1)}`).join(" ");

  const areaPts = `${padL},${y(0)} ${cumPts} ${lastX},${y(0)}`;
  const barW = Math.max(2, (W - padL - padR) / days * 0.42);
  ...
  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ display: "block" }}>
      ...
      <g key={i}>
          <line x1={padL} y1={y(D.totals.budget * f)} x2={W - padR} y2={y(D.totals.budget * f)}
        stroke="rgba(106,95,192,.18)" strokeWidth="1" strokeDasharray={f === 1 ? "0" : "2 4"} />
          <text x={W - padR} y={y(D.totals.budget * f) - 3} textAnchor="end"
        fill="var(--ink-3)" fontSize="8.5" fontFamily="var(--font-body)" letterSpacing=".06em">
            {f === 1 ? "BUDGET" : (D.totals.budget * f / 1000).toFixed(1) + "k"}
          </text>
        </g>
      )}
      {/* daily bars */}
      {showBars && D.daily.map((d, i) => {
      ...
      {/* prior cycle */}
      {showLast && <polyline points={lastPts} fill="none" stroke="rgba(143,125,255,.55)" strokeWidth="1.4" strokeDasharray="4 4" />}
      {/* budget pace */}
      {showPace && <polyline points={pacePts} fill="none" stroke="var(--indigo-neon)" strokeWidth="1.2" strokeDasharray="5 5" opacity=".8" />}
      {/* cumulative area + line */}
      {showArea && <polygon points={areaPts} fill="url(#cumfill)" />}
      <polyline points={cumPts} fill="none" stroke="var(--neon)" strokeWidth="2" strokeLinejoin="round"
      style={{ filter: "drop-shadow(0 0 3px var(--neon))" }} />
      {/* today marker */}
      <line x1={lastX} y1={padT - 6} x2={lastX} y2={H - padB} stroke="rgba(255,59,46,.4)" strokeWidth="1" strokeDasharray="2 3" />
      <circle cx={lastX} cy={lastY} r="3" fill="var(--neon-white)" style={{ filter: "drop-shadow(0 0 4px var(--neon))" }} />
    </svg>);

}

/* ---- SavingsDial: arc gauge for savings progress ---- */
function SavingsDial({ size = 132, saved, target, projected }) {
  const D = window.PHOSK;
