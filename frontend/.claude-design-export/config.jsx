...
function useCfg() {
  const read = window.__phoskReadCfg || (() => ({}));
  const write = window.__phoskWriteCfg || (() => {});
  const init = () => {
    const stored = read();
    const out = { ...CFG_DEFAULTS };
    for (const k in CFG_DEFAULTS) if (k in stored) out[k] = stored[k];
    return out;
  };
  ...
  const set = (keyOrEdits, val) => {
    ...
    // Persist the changed preference(s) to the backend; topDateFmt also lives in /config.
    window.phoskApi.patch('/settings/preferences', edits);
    if ('topDateFmt' in edits) window.phoskApi.patch('/config', { topDateFmt: edits.topDateFmt });
  };
  const reset = () => {
    setCfg({ ...CFG_DEFAULTS });
    try { localStorage.removeItem(window.PHOSK_CFG_KEY || "phosk.cfg"); } catch (e) {}
    window.dispatchEvent(new CustomEvent("phoskcfg", { detail: { ...CFG_DEFAULTS } }));
    window.phoskApi.del('/settings/preferences');
  };
...
function ConfigPage() {
  ...
  return (
    <div className="pk" style={{ height: "100vh", minHeight: 0 }}>
      ...
      <window.ScannerBg className="pk-bg" seed={117} shapes={[
            ...
            <div className="cfg-wrap">
              ...
              <div className="cfg-ribbon">
                ...
                <div className="cfg-rib">
                  ...
                  <div className="s">{D.account.engine} · {D.account.model}</div>
                </div>
              </div>
